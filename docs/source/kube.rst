Scaling [hysyssolutiondemo-3f10-ml] With Kubernetes
===========================
...
Based on your TML solution [hysyssolutiondemo-3f10-ml] - if you want to scale your application with Kubernetes - you will need to apply the following YAML files.

.. list-table::

   * - **YML File**
     - **Description**
   * - :ref:`hysyssolutiondemo-3f10-ml.yml`
   ...
   * - :ref:`qdrant.yml`
     - This is the Qdrant deployment YAML.
       ...
       applied if using Step 9 DAG.
   * - :ref:`nginx-ingress-hysyssolutiondemo-3f10-ml.yml`
     - If you are scaling your TML solution you must

       apply the nginx-ingresshysyssolutiondemo-3f10-ml.yml; this yaml is 
...
.. code-block:: YAML
   ...
   kubectl apply -f secrets.yml -f mysql-storage.yml -f mysql-db-deployment.yml -f hysyssolutiondemo-3f10-ml.yml

hysyssolutiondemo-3f10-ml.yml
------------------------

.. important::
   Copy and Paste this YAML file: hysyssolutiondemo-3f10-ml.yml - and save it locally.
...
.. code-block:: YAML
   ...
   ################# hysyssolutiondemo-3f10-ml.yml
     ...
     kind: Deployment
     metadata:
       name: hysyssolutiondemo-3f10-ml
     spec:
       selector:
         matchLabels:
           app: hysyssolutiondemo-3f10-ml
       ...
       template:
         ...
         spec:
           containers:
           - name: hysyssolutiondemo-3f10-ml
             image: maadsdocker/hysyssolutiondemo-3f10-ml-amd64:latest
             volumeMounts:
             - name: dockerpath
             ...
             - name: TSS
               value: '0'
             - name: PROJECTNAME
               value: 'hysyssolutiondemo-3f10'                              
             - name: SOLUTIONNAME
               value: 'hysyssolutiondemo-3f10-ml'
             - name: SOLUTIONDAG
               value: 'solution_preprocessing_ml_dag-hysyssolutiondemo-3f10'
             - name: GITUSERNAME
               value: 'smaurice101'
             - name: GITREPOURL
             ...
             - name: step5independentvariables # STEP 5 independent variables can be adjusted here.  
               value: 'psia_preprocessed_Avg,Choke_preprocessed_Avg,V-200_preprocessed_Avg,F_preprocessed_Avg,PIC-100_preprocessed_Avg,LIC-101_preprocessed_Avg'                                                                              
             ...
             - name: step1solutiontitle # STEP 1 solutiontitle field can be adjusted here. 
               value: 'TML HYSYS Solution: Oil and Gas Demo'                              
             - name: step1description # STEP 1 description field can be adjusted here. 
               value: 'This solution is using TML to process data from HYSYS and Perform Entity-Based Machine Learning for Each Well'                                          
             - name: KUBEBROKERHOST
               value: 'kafka-service:9092'         
             - name: KAFKABROKERHOST
               value: 'pkc-3w22w.us-central1.gcp.confluent.cloud:9092'                              
           volumes: 
           - name: dockerpath
             hostPath:
   ...
   ---
     ...
     kind: Service
     metadata:
       name: hysyssolutiondemo-3f10-ml-visualization-service
       labels:
         app: hysyssolutiondemo-3f10-ml-visualization-service
     spec:
       type: ClusterIP
       ports:
...
.. code-block::
   ...
   kubectl port-forward deployment/hysyssolutiondemo-3f10-ml 6060:6060

After you forward the ports then copy/paste the viusalization URL below and run your dashboard.

.. code-block::

   http://localhost:6060/iot-failure-machinelearning.html?topic=iot-preprocess,iot-ml-prediction-results-output&offset=-1&groupid=&rollbackoffset=400&topictype=prediction&append=0&secure=1
...
.. important::
   ...
   **STEP 4:  Apply nginx-ingress-hysyssolutiondemo-3f10-ml.yml to your kubernetes cluster.  First you need to save it locally then apply it:**

nginx-ingress-hysyssolutiondemo-3f10-ml.yml
-------------
   ...
   .. code-block::
      ...
      kubectl apply -f nginx-ingress-hysyssolutiondemo-3f10-ml.yml
